...
<div class="mt-5 border-2 rounded-2xl transition-all hover:shadow-xl">
        ...
        <div class="flex items-start gap-10">
            <div class="user-info px-2">
                ...
                <div class="text-muted">
                    <p class="font-bold">{{ $journey->seats - $journey->used_seats }} seats left</p>
                </div>
            </div>
            <div class="price">
                <h1 class="text-3xl font-black">{{ $journey->price }} RSD</h1>
                <p class="font-bold">{{ date("d. M. Y.", strtotime($journey->departure_time)) }}</p>
            </div>
        </div>
    ...
    <div class="timeline flex justify-start rounded-2xl py-3 my-3 mx-2 bg-[#eff1f9]">
        <ul class="flex w-full px-3 gap-2">
            <li class="location">
                ...
                <p class="text-sm text-muted">{{ date("H:i", strtotime($journey->departure_time)) }}</p>
            </li>
            <li class="stop flex-grow">
                <div class="time">
                    <div class="eta flex justify-center items-center">
                        <h1 class="bg-accent rounded-2xl px-4 text-white">{{ $journey->duration }}</h1>
                    </div>
                </div>
            </li>
